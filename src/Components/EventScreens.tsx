...
function getPhaseColor(p: number) {
  return (PHASES.find((ph) => p <= ph.end) ?? PHASES[PHASES.length - 1]).color;
}
...
// ── Slide 1: Timer ─────────────────────────────────────────────────────────────
function SlideTimer({ progress }: { progress: number }) {
  const timeLeft = Math.round((1 - progress) * 18 * 3600);
  const color = getPhaseColor(progress);
...
// ── Slide 3: Journey ───────────────────────────────────────────────────────────
function SlideJourney({ progress }: { progress: number }) {
  const colors = [
    C.teal,
    C.blue,
    C.amber,
    ...
    >
      ...
      >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "7px" }}>
            <div
              style={{
                ...
                animation: isPlaying
                  ? "hqPulse 1.2s ease-in-out infinite"
                  : "none",
              }}
            />
            <span
        ...
        <div style={{ flex: 1, overflow: "hidden" }}>
          ...
          {currentSlide === 1 && <SlideTimer progress={liveProgress} />}
          {currentSlide === 2 && <SlideFormat />}
          {currentSlide === 3 && <SlideJourney progress={liveProgress} />}
        </div>
