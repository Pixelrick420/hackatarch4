    ...
    useEffect(() => {
      ...
      const animate = (timestamp: number) => {
        ...
        bounds.minX = Math.max(
          0,
          Math.floor(bounds.minX / gridSize) * gridSize,
        );
        bounds.maxX = Math.min(
          canvas.width,
          Math.ceil(bounds.maxX / gridSize) * gridSize,
        );
        bounds.minY = Math.max(
          0,
          Math.floor(bounds.minY / gridSize) * gridSize,
        );
        bounds.maxY = Math.min(
          canvas.height,
          Math.ceil(bounds.maxY / gridSize) * gridSize,
        );
        ...
        for (let i = 0; i < allMetaballs.length; i++) {
          radiusSquaredCache[i] =
            allMetaballs[i].radius * allMetaballs[i].radius;
        }
